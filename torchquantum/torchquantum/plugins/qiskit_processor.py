...
class QiskitProcessor(object):
    ...
    def get_noise_model(self, name):
        if name in IBMQ_NAMES:
            if self.provider is None:
                raise RuntimeError(
                    "An IBM Quantum provider is required to load an IBM noise model."
                )
    ...
    def get_coupling_map(self, name):
        if name in IBMQ_NAMES:
            if self.provider is None:
                raise RuntimeError(
                    "An IBM Quantum provider is required to load an IBM coupling map."
                )
            backend = self.provider.get_backend(name)
    ...
    def get_basis_gates(self, name):
        if name in IBMQ_NAMES:
            if self.provider is None:
                raise RuntimeError(
                    "An IBM Quantum provider is required to load IBM basis gates."
                )
            backend = self.provider.get_backend(name)
    ...
    def qiskit_init(self):
        ...
        if self.backend is None:
            needs_provider = self.use_real_qc or any(
                name in IBMQ_NAMES
                for name in (
                    self.backend_name,
                    self.noise_model_name,
                    self.coupling_map_name,
                    self.basis_gates_name,
                )
            )
